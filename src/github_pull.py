...
def get_github_single_file(
    repo_owner: str, repo_name: str, commit: str, file_path: str, download_to: str, token = None):
    ...
    if response.ok and response.status_code == 200:
        ...
        raise ValueError(
            f"Network Failed to Download File! Code: {response.status_code}"
        )
        ...
        for item in data:
            if item["type"] == "dir":
                ...
                content = get_github_file_content(
                    repo_owner, repo_name, subdir_path, file_name
                )
